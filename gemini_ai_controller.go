...
func ParsePDFHandler(c *gin.Context) {
	...
	textContent := genai.Content{
		Parts: []*genai.Part{
			...
			genai.NewPartFromText("The PDF file is a portfolio. Extract it to a json as the following { \"broker\":\"Broker\" \"portfolioCode\": \"1234\", \"date\":\"2025-10-01\" \"financialAmount\": 123456.78, \"currency\":\"BRL\" \"positions\": [ { \"assetType\": \"Cash\", \"assetCode\": \"TEST123\", \"assetDescription\":\"The stock TEST1234\", \"currency\":\"USD\", \"quantity\": 100.00, \"price\": 10.00, \"financialAmount\": 1000.0, \"maturityDate\":\"2025-05-06\"  } ]  }. The broker code must be the broker which generated the file, on upper case, with _ instead of spaces between words and no special characters."),
			...
			genai.NewPartFromText("Be careful. Some position lines are consolidating an assetType or sector, and also the quantity is not always called quantity. If you cannot find it, try to get the best match, but all the numbers must be in the file."),
		},
		Role: "user",
	}
